...
function drawCard(doc, x, y, width, height, horizontalMargin) {
  ...
  const verticalMargin =
    (height - imageHeight - footerSize - maxInnerWidth) / 4;
  ...
  // Draw inner box.
  doc
    .moveTo(x + horizontalMargin, y + innerY)
  ...
  for (var i = 0; i < 4; i++) {
    doc
      .moveTo(x + horizontalMargin + boxSize * (i + 1), y + innerY)
      .lineTo(
        x + horizontalMargin + boxSize * (i + 1),
        y + innerY + innerHeight
      )
      .stroke();

    doc
      .moveTo(x + horizontalMargin, y + innerY + boxSize * (i + 1))
  ...
  for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
      ...
      doc
        ...
        .text(
          `${num}`,
          x + horizontalMargin + i * boxSize,
          y +
            innerY +
            j * boxSize +
            (boxSize - numberSize) / 2 +
            mmToPdfPoints(1),
          {
            width: boxSize,
            align: "center",
  ...
  doc.font("Helvetica").fillColor("#b95c27").text(linkText, linkX, linkY, {
    width: innerWidth,
    align: "right",
  });
  ...
  doc.link(
    linkX + innerWidth - linkWidth,
    linkY,
    linkWidth,
    linkHeight,
    "https://suppkultur.org/"
  );
}
...
exports.generatePDF = async (count) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (count > 1) {
      drawCard(
        doc,
        leftMargin + sheetWidth + sheetMargin,
        topMargin,
        sheetWidth,
        sheetHeight,
        margin
    ...
    if (count > 2) {
      drawCard(
        doc,
        leftMargin,
        topMargin + sheetHeight + sheetMargin,
        sheetWidth,
        sheetHeight,
        margin
      );
